...
for (const pair of K.pairs.list)
{
	...
	for (const type of K.types.list)
	{
		...
		///
		if(!index_names.includes('idx_species')) {
			results.indexes.push(`${name}.idx_species`)
			if (context.isProduction) {
				console.debug(`Ensuring index idx_species for collection ${name}.`)
			}
			collection.ensureIndex({
				name: 'idx_species',
				type: "persistent",
				fields: ["properties.eu.species_list[*]"],
				estimates: true,
				cacheEnabled: false,
				deduplicate: false,
				sparse: false,
		...
		// Create unit index.
		///
		if(!index_names.includes('idx_units')) {
			results.indexes.push(`${name}.idx_units`)
			if (context.isProduction) {
				console.debug(`Ensuring index idx_units for collection ${name}.`)
			}
			collection.ensureIndex({
				name: 'idx_units',
				type: "persistent",
				fields: ["properties.eufgis.gcu_id_number_list[*]"],
				estimates: true,
				cacheEnabled: false,
				deduplicate: false,
				sparse: false,
